...
# add-internal-skill は skill add を fillin-inc/internal-skills 固定で呼ぶショートカット
...
(( $+commands[mo] )) && alias mog='git rev-parse --show-toplevel >/dev/null 2>&1 || { echo "Error: not in git repository" >&2; false; } && mo --target="$(basename "$(root_path)")"'
if (( $+commands[go] )); then
...
(( $+commands[opencode] )) && alias ocode='opencode --auto'
(( $+commands[opencode2] )) && alias ocode2='opencode2 --auto'
